...
import { SignableDocument } from '@sign/js/common/document_signable';
  ...
  get_pdfiframe_class: function () {
    const SignablePDFIframe = this._super.apply(this, arguments).extend({
      ...
      registerCreatedSignItemEvents($signatureItem, type, isSignItemEditable) {
        ...
        if (type.item_type === "sequential") {
          $signatureItem.on("focus", (e) => {
            getSequentialNumberForElementType(type.name)
              .then(data => {
                const numberFormatted = String(data.sequentialNumber).padStart(data.padding, '0');            
                var sequentialNumberComplete
                if (typeof data.prefix === 'string' && typeof data.suffix === 'string') {
                  sequentialNumberComplete = `${data.prefix}${numberFormatted}${data.suffix}`;
                }
                if (typeof data.prefix === 'string') {
                  sequentialNumberComplete = `${data.prefix}${numberFormatted}`;
                } else if(typeof data.suffix === 'string') {
                  sequentialNumberComplete = `${numberFormatted}${data.suffix}`;
                }
                $signatureItem.val(sequentialNumberComplete);
              });          
          });
        }
...
function getSequentialNumberForElementType(signItemType) {
return rpc.query({
    model: 'sign.item.type',
    method: 'search_read',
    domain: [['name', '=', signItemType]],
    fields: ['sequence_id'],
})
    .then(itemType => {
    if (itemType.length > 0) {
        const sequence_id = itemType[0].sequence_id[0];

        return rpc.query({
        model: 'ir.sequence', 
        method: 'search_read',
        domain: [['id', '=', sequence_id]], 
        fields: ['number_next', 'prefix', 'suffix', 'padding'],
        })
        .then(sequence => {
            if (sequence.length > 0) {
            const sequentialNumber = sequence[0].number_next;
            const prefix = sequence[0].prefix; 
            const suffix = sequence[0].suffix; 
            const padding = sequence[0].padding;

            const nextNumeroSecuencial = sequentialNumber + 1
            return rpc.query({
                model: 'ir.sequence',
                method: 'write',
                args: [[sequence_id], { 'number_next': nextNumeroSecuencial }],
            })
                .then(() => {
                return {
                    sequentialNumber,
